...
<!--[if IE 7]>
    ...
    div#categorylist div#title .detailtitle{
      margin-top: -6px;
      margin-left: 3px;
    }
    div.listblock .attention{
      margin-left: 0px;
    }
    div.listblock .listcomment {
      margin-top: 50px;
      width: 400px;
    }
    div#categorylist div.cartinfo {
      position: relative;
      margin-top: -80px;
      margin-left: 50px;
      margin-bottom: 20px;
    ...
    div.listblock .listbuttons {
      margin-left: 180px;
    }
    div.listbuttons .btnbox {
    ...
    <!--▼商品ここから-->
       ...
       <ul class="status_icon">
         <!--{section name=flg loop=$arrProducts[cnt].product_flag|count_characters}-->
           <!--{if $arrProducts[cnt].product_flag[flg] == "1"}-->
             ...
             <li><img src="<!--{$TPL_DIR}--><!--{$arrSTATUS_IMAGE[$key]}-->" width="70" height="23" alt="&lt;!--{$arrSTATUS[$key]}--&gt;"/></li>
     ...
     <div class="listphoto">
        ...
        <a href="<!--{$smarty.const.DETAIL_P_HTML}--><!--{$arrProducts[cnt].product_id}-->" class="over"><!--商品写真--><img src="<!--{$smarty.const.IMAGE_SAVE_URL|sfTrimURL}-->/<!--{$arrProducts[cnt].main_list_image}-->" alt="<!--{$arrProducts[cnt].name|escape}-->" class="picture" /></a>            
         ...
         <!--{assign var=quantity value=quantity`$id`}--><br />
           ...
           <input type="text" name="<!--{$quantity}-->" size="3" class="box54" value="<!--{$arrForm[$quantity]|default:1}-->" maxlength="<!--{$smarty.const.INT_LEN}-->" style="<!--{$arrErr[$quantity]|sfGetErrorColor}-->" />
             ...
             <br />
             <!--{/if}-->
           </dt><span class="attention"><!--{$arrErr[$quantity]}--></span>
         </dl></div>
         ...
         <div class="listbuttons">
             ...
             <a href="<!--{$smarty.server.REQUEST_URI|escape}-->#product<!--{$id}-->" onclick="fnChangeAction('<!--{$smarty.server.REQUEST_URI|escape}-->#product<!--{$id}-->'); fnModeSubmit('cart','product_id','<!--{$id}-->'); return false;" onmouseover="chgImg('<!--{$TPL_DIR}-->img/products/b_cartin_on.gif','cart<!--{$id}-->');" onmouseout="chgImg('<!--{$TPL_DIR}-->img/products/b_cartin.gif','cart<!--{$id}-->');">
               <img src="<!--{$TPL_DIR}-->img/products/b_cartin.gif" width="90" height="20" alt="カゴに入れる" name="cart<!--{$id}-->" id="cart<!--{$id}-->" /></a>
             </div>
         ...
         <!--{assign var=name value="detail`$smarty.section.cnt.iteration`"}-->
           <a href="<!--{$smarty.const.DETAIL_P_HTML}--><!--{$arrProducts[cnt].product_id}-->" onmouseover="chgImg('<!--{$TPL_DIR}-->img/products/b_detail_on.gif','<!--{$name}-->');" onmouseout="chgImg('<!--{$TPL_DIR}-->img/products/b_detail.gif','<!--{$name}-->');">
            <img src="<!--{$TPL_DIR}-->img/products/b_detail.gif" width="82" height="20" alt="詳しくはこちら" name="<!--{$name}-->" id="<!--{$name}-->" /></a>
         </span>
